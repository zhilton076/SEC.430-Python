...
class ChatClient(EasyFrame):
    ...
    def __init__(self):
        ...
        self.outputLabel = self.addLabel(text = "Phone Book Server Contents:",
                                         row = 0, column = 0,
                                         columnspan = 2,
                                         sticky = "NSEW")
        ...
        self.inputArea = self.addTextArea("", row = 3, column = 0,
                                          columnspan = 2,
                                          width = 50, height = 4)
        self.sendButton = self.addButton(text = "Respond",
                                         row = 4, column = 0,
                                         command = self.send,
                                         state = "disabled")
    ...
    def send(self):
        ...
        reply = decode(self.server.recv(BUFSIZE), CODE)
        if reply == "File cannot be found. Disconnecting from server... ":
            self.disconnect()
        self.outputArea.setText(reply)
